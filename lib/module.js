...
const removeSsrAttribute = (html) => {
  return html.replace(/ data-n-head="ssr" src="https:\/\/pagead2.googlesyndication.com\/pagead\/js\/adsbygoogle.js"/g, '')
}
...
module.exports = function nuxtAdSense (moduleOptions = {}) {
  ...
  // Normalize options
  options.headScript = Boolean(options.headScript)
  ...
  if (!this.options.headScript === false) {
    this.options.headScript = true
  }
  ...
  // Google ads chokes on the data-n-head="ssr" attribute, so remove it for 
  this.nuxt.hook('render:route', async (url, page, { req, res }) => {
    page.html = removeSsrAttribute(page.html)
  })
